...
class YelpEval(BinaryClassifierEval):
    def __init__(self, task_path, seed=1111):
        ...
        text, label =  dataset['train']['text'], dataset['train']['label']
        neg, pos=[], []
...
class RTTMEval(BinaryClassifierEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : Rotten tomatoes *****\n\n')
...
class SST2Eval(BinaryClassifierEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : sst2 *****\n\n')
        dataset = load_dataset("sst2")
        text, label =  dataset['train']['sentence'], dataset['train']['label']
        neg, pos=[], []
        for idx, value in enumerate(label):
            if value == 0:
                neg.append(text[idx])
