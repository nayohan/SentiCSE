...
from senteval.binary import CREval, MREval, MPQAEval, SUBJEval, YelpEval, IMDBEval, RTTMEval, SST2Eval
...
from senteval.sts import STSEval
from senteval.rank import ImageCaptionRetrievalEval
from senteval.clsts import CLSTSEval
from senteval.sentiment import SentimentEval
...
class SE(object):
    ...
    def eval(self, name):
        ...
        # elif name == 'SST2':
        #     self.evaluation = SSTEval(tpath + '/downstream/SST/binary',seed=self.params.seed)
        elif name == 'SST5':
            ...
            self.evaluation = SentimentEval('imdb', nclasses=2, seed=self.params.seed)
        elif name == 'YELP2':
            self.evaluation = SentimentEval('yelp_polarity', nclasses=2, seed=self.params.seed)
        elif name == 'YELP5':
            self.evaluation = SentimentEval('yelp_review_full', nclasses=5, seed=self.params.seed)
        elif name == 'MR':
            self.evaluation = SentimentEval('rotten_tomatoes', nclasses=2, seed=self.params.seed)
        elif name == 'SST2':
            self.evaluation = SentimentEval('sst2', nclasses=2, seed=self.params.seed)

        elif name == 'imdb':
            self.evaluation = CLSTSEval(tpath + '/downstream/SENTIMENT/', name, seed=self.params.seed)
        elif name == 'yelp2':
            self.evaluation = CLSTSEval(tpath + '/downstream/SENTIMENT/', name, seed=self.params.seed)
        elif name == 'yelp5':
            self.evaluation = CLSTSEval(tpath + '/downstream/SENTIMENT/', name, seed=self.params.seed)
