...
class PyTorchClassifier(object):
    ...
    def fit(self, X, y, validation_data=None, validation_split=None,
            test_x =None, test_y=None, early_stop=True, tuning= True):
        self.nepoch = 0
        bestaccuracy = -1
        stop_train = False
        early_stop_count = 0
        ...
        while not stop_train and self.nepoch <= self.max_epoch:
            print('reg:',self.l2reg, 'now_epoch:', self.nepoch)
            if tuning == True :
                self.trainepoch(trainX, trainy, epoch_size=self.epoch_size)
            else:
                self.validepoch(trainX, trainy,devX, devy,test_x,test_y,epoch_size=self.epoch_size)
            accuracy = self.score(devX, devy)
            print(f'===== End of Training: acc: {accuracy} ====')
    ...
    def validepoch(self, X, y,devX, devy,test_x,test_y, epoch_size=1):
        self.model.train()
        ep = 1
        for _ in range(self.nepoch, self.nepoch + epoch_size):
            ...
            for i in range(0, len(X), self.batch_size):
                ...
                # Update parameters
                self.optimizer.step()
            test_acc = self.score(test_x, test_y)
            valid_acc = self.score(devX, devy)
            print('===============================')
            print('Training Epoch is ', ep)
            print('Validation Score is ',valid_acc)
