...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, training_args, others = parser.parse_args_into_dataclasses(return_remaining_strings=True)
    ...
    if len(column_names) == 2:
        ...
        # Pair dataset with quadruple sentences
    ...
    def prepare_features(examples):
        ...
        sent_features = get_senti_type(sent_features) # add senti_type    
        sent_features = mask_tokens_map(sent_features) # add senti_mlm using senti_type
        features = {}
        if sent3_cname is not None:
            for key in sent_features:
                features[key] = [[sent_features[key][i], sent_features[key][i+total], sent_features[key][i+total*2],sent_features[key][i+total*3]] for i in range(total)] # 0, 1000, 2000, 3000
        elif sent2_cname is not None:
            for key in sent_features:
                features[key] = [[sent_features[key][i], sent_features[key][i+total], sent_features[key][i+total*2]] for i in range(total)] # 0, 1000, 2000
        else:
            for key in sent_features:
    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            ...
            if not model_args.do_mlm:
                del batch["mlm_input_ids"]
                del batch["mlm_labels"]
